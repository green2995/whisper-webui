...
from src.custom_utils.aws_utils import download_s3_file, upload_file_to_s3

LANGUAGE_NONE_ALTERNATIVE_FOR_BATCH = "auto"
MODIFIED_LANGUAGE_CHOICE_FOR_BATCH = [LANGUAGE_NONE_ALTERNATIVE_FOR_BATCH, *sorted(get_language_names())]
...
def cli():
    ...
    parser.add_argument("--language", type=str, default=app_config.language, choices=MODIFIED_LANGUAGE_CHOICE_FOR_BATCH, \
    ...
    parser.add_argument("--s3_audio_uri", type=str, help="s3 audio uri", required=True)
    parser.add_argument("--s3_output_prefix", type=str, help="s3 output prefix", required=True)
    parser.add_argument("--id", type=str, help="id to identify output", required=True)

    args = parser.parse_args().__dict__

    if args["language"] == LANGUAGE_NONE_ALTERNATIVE_FOR_BATCH:
        args["language"] = None
    ...
    s3_audio_uri: str = args.pop("s3_audio_uri")
    output_id: str = args.pop("id")
    s3_output_prefix: str = args.pop("s3_output_prefix")

    audio_path = download_s3_file(s3_audio_uri)
    ...
    result = transcriber.transcribe_file(model, audio_path, temperature=temperature, vadOptions=vadOptions, **args)
    transcriber.write_result(result, output_id, output_dir, highlight_words)

    transcriber.close()

    upload_file_to_s3(f"{output_dir}/{output_id}-result.json", f"{s3_output_prefix}/{output_id}.json")
